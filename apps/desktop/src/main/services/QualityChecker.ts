...
export class QualityChecker {
  async evaluate(source: string, target: string, _lexicon: LexiconEntry[]): Promise<QualityReport> {
    // Version simplifiee sans IA pour le MVP : scoring heuristique
    const sourceLen = source.length
    const targetLen = target.length
    const lengthRatio = sourceLen === 0 ? 1 : targetLen / sourceLen
    const lengthScore = Math.max(0, Math.min(100, 100 - Math.abs(lengthRatio - 1) * 100))
