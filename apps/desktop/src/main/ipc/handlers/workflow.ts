...
export function registerWorkflowHandlers(): void {
  ipcMain.handle('workflow:start', async (_event, projectPath: string, chapterId?: string) => {
  ...
  ipcMain.handle('workflow:retry-step', async (_event, jobId: string, stepId: string) => {
    await workflowEngine.retryStep(jobId, stepId)
    return { ok: true }
  })

  ipcMain.handle('workflow:retry-from', async (_event, jobId: string, stage: WorkflowStage) => {
