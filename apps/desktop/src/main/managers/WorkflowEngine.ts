...
import { createProjectDatabase, runMigrations, type ProjectDatabase } from '../db/connection.js'
...
class WorkflowRunner extends EventEmitter {
  ...
  constructor(
    ...
    aiRouter.register(new OllamaProvider('ollama-default', 'Ollama local', defaultModel, ollamaHost))
  ...
  private async applyAgentOutput(stage: WorkflowStage, output: AgentOutput): Promise<void> {
    ...
    if (output.text && ['lexicon', 'grammar', 'style', 'polish'].includes(stage)) {
...
export class WorkflowEngine {
  ...
  async start(projectPath: string, chapterId?: string): Promise<Job> {
    const runner = new WorkflowRunner(projectPath, this.settings, (p) => this.emitProgress(p))
