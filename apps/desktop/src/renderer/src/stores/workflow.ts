...
export const useWorkflowStore = defineStore('workflow', () => {
  ...
  async function start(projectPath: string, chapterId?: string): Promise<Job> {
    loading.value = true
    try {
      const job = await window.novelTradAPI.invoke<Job>('workflow:start', projectPath, chapterId)
  ...
  return { activeJobs, progress, loading, start, pause, resume, cancel, list, retryStep, retryFrom }
})
