Warning: truncated output (original token count: 73729)
Total output lines: 4429
...
- **Constante partagée** : création de `packages/shared/src/constants/languages.ts` (`SOURCE_LANGUAGES`, `TARGET_LANGUAGES`) — unifie 3 sources de vérité divergentes (HomeView, WizardDialog, SettingsView n'avait aucune liste).
- **Fichiers modifiés** : `SettingsView.vue` (modèles + langues en `<select>`), `HomeView.vue`, `WizardDialog.vue` (unification), `constants/languages.ts` + `index.ts` (nouveau), `index.ts` (export).
- **Vérification** : `type-check` 0 erreurs, `npm run test` **961/961 passés** (67 files, +2), `lint` 0 erreurs.

## Request — SDD Depth Audit + Code Conformance (2026-07-07)
- **Audit de profondeur SDD** — vérification des 5 domaines réclamés « manquants » par une analyse externe (Multi-Agent, Pipeline, DB, UI, Plugins/Providers).
- **Méthodologie** : lecture intégrale des volumes 03, 04, 06, 07, 08, 15 + vérification croisée (05, 10, 17).
- **Résultat audit** : analyse externe **largement incorrecte** (reposait sur un accès incomplet au dépôt). ~31 faux signalements dissipés, 9 écarts réels identifiés.
- **Phase 1 (docs)** : 9 écarts spec corrigés dans les volumes 03/04/06/07/15 + doc audit `docs/audit/SDD_DEPTH_AUDIT_2026-07-07.md`.
- **Phase 2 (code)** : 6 écarts code réels implémentés (G1, G2, G3, G5, G7, G8). G4/G6/G9 étaient spec-only.
- **Vérification finale** : `npm run type-check` 0 erreurs, `npm run test` **959/959 passés** (66 files, +19 tests vs 940), `npm run lint` 0 erreurs.

### Implémentation code (Phase 2)

| Gap | Description | Fichiers | Tests |
|-----|-------------|----------|-------|
| **G1** | Per-step timeout (`stepTimeoutMs`) | `types/index.ts`, `schemas/index.ts`, `WorkflowEngine.ts`, `stores/settings.ts` | 4 (`workflow-timeout.spec.ts`) |
| **G2** | HTTP 429 handling (Retry-After) | `OllamaProvider.ts`, `OpenAiCompatibleProvider.ts` | 4 (`providers-429.spec.ts`) |
| **G3** | Cost estimation + `job.cost_usd` | `types/index.ts`, `schemas/index.ts`, `AiRouter.ts`, `JobRepository.ts`, `WorkflowEngine.ts` | 4 (`cost-estimation.spec.ts`) |
| **G5** | DB indexes + `cost_usd` column | `migrations/013_index_cost.sql`, `db-migrations.spec.ts` | +1 (existante) |
| **G7** | ProjectView loading/empty/error states | `ProjectView.vue`, `stores/project.ts` | 4 (`project-view.spec.ts`) |
| **G8** | apiVersion compatibility check | `types/plugin.ts`, `PluginHost.ts` | +3 (`plugin-host.spec.ts`) |

**Décisions de design** :
- G2 : throttling proactif SKIPPÉ (user decision — seul OllamaProvider est câblé, le throttling serait du code dormant). Seul le 429 réactif (Retry-After) est implémenté.
- G3 : coûts stockés en settings (`modelCosts` record), pas en DB — évite une nouvelle colonne `models.metadata` + ModelRepository.
- G5 : migration numérotée `013` (012 pris). Seuls 2 indexes réellement manquants (`paragraphs.status`, `prompts.agent_id`) — les 2 autres préexistent.
- G8 : `apiVersion` est sur l'instance plugin (pas le manifest), vérifié après `import()`.

## Request — SDD Depth Audit (2026-07-07)
- **Audit de profondeur SDD** — vérification des 5 domaines réclamés « manquants » par une analyse externe (Multi-Agent, Pipeline, DB, UI, Plugins/Providers).
- **Méthodologie** : lecture intégrale des volumes 03, 04, 06, 07, 08, 15 + vérification croisée (05, 10, 17).
- **Résultat** : analyse externe **largement incorrecte** (reposait sur un accès incomplet au dépôt). ~31 faux signalements dissipés, 9 écarts réels identifiés et corrigés.
- **Type** : docs uniquement (Markdown), 0 test impacté, 0 code modifié.
- **Vérification** : `npm run type-check` 0 erreurs, `npm run test` 940/940 passés (62 files).
...
| `apps/desktop/tests/uni…13729 tokens truncated…k to heuristic behavior when LLM fails | ✅ |
...
## Handoff Note (2026-07-08) — v1.4 : Boucle de révision pro + Summarizer + câblage plugin

### Contexte
Audit du SDD vs demande initiale (app de traduction multi-format avec workflow multi-agent de révision pour approcher une traduction pro). L'audit a révélé un pipeline 10 passes fonctionnel mais 3 écarts :
1. `PluginHost` ↔ `WorkflowEngine` non câblé (plugins d'agents inertes).
2. Pas de vraie boucle review→correction ciblée (seul `retryWeakestStep` = retry global).
3. Pas de Summarizer cross-chapitre (cohérence long-terme).

### Livrables (toutes phases A→E1 terminées)

**Phase A — Doc SDD** :
- `06-Database.md` : entériné `node-sqlite3-wasm` (WONTFIX migration) + `history_snapshots`.
- `25-Prompt-Book.md` : corrigé coquilles caractères ESC (xport→export, ranslate→translate, etc.).
- `07-Workflow.md` + `08-Agents.md` + `25-Prompt-Book.md` : documenté stages `review`/`revise` + Summarizer transverse (pipeline 12 passes).

**Phase B — Wiring** :
- `WorkflowEngine.ts` : `setPluginHost(pluginHost)` injecte `getPluginAgent` dans `AgentFactory` + `setPluginProviderResolver` dans `AiRouter`.
- `index.ts` : `workflowEngine.setPluginHost(pluginHost)` après init PluginHost.
- Commentaire worker threads obsolète corrigé (confirmés activés par défaut `true`).

**Phase C1 — Boucle Review→Revise** :
- Types : `ReviewReport`, `ReviewIssue`, `ReviewCategory` + `WorkflowStage` étendu (`review`, `revise`).
- Agents : `ReviewAgent.ts` (LLM jsonMode + normalisation issues + score par pénalité), `ReviseAgent.ts` (applique ReviewReport, skip si vide, refus éthique).
- Prompts : `review.system.ts`, `revise.system.ts`.
- Migration `014_review_stage.sql` : table `review_reports` + registre agents.
- `AgentFactory` + `STAGES` (12 passes) + `getActiveStages()` (désactivable via `reviewLoopEnabled`).

**Phase C2 — Summarizer transverse** :
- Types : `ChapterSummary`, `NovelSummary`.
- `SummarizerAgent.ts` (transverse, appelé après export).
- Migration `015_summaries.sql` : `chapter_summaries` + `novel_summaries`.
- `SummaryRepository.ts` : CRUD avec upsert + versionnage.
- Injection `novelSummary` dans `buildAgentInput` (translate/style/polish/review).
- `summarizeChapter()` appelé après stage `export` (comme `storeEmbeddingsForChapter`).

**Phase C3 — UI** :
- `SettingsView.vue` : toggles "Boucle de révision pro" + "Summarizer".
- `WorkflowView.vue` : affichage `ReviewReport` (issues paragraphe-par-paragraphe avec sévérité/catégorie) + labels review/revise.

**Phase D — Validation en→fr** :
- Fixture `tests/fixtures/sample_en_chapter.md` (roman fantasy en, ~8 paragraphes).
- Test d'intégration `review-loop-integration.spec.ts` : Review→Revise enchaîne les données, Summarizer produit un résumé cohérent.

### Métriques
| Métrique | Valeur |
|----------|--------|
| Tests | 981 passés (71 suites), 0 failed |
| Type-check | 0 errors (main tsc + vue-tsc) |
| Lint | 0 errors (18 warnings préexistants) |
| Nouvelles migrations | 014_review_stage.sql, 015_summaries.sql |
| Nouveaux fichiers | ReviewAgent, ReviseAgent, SummarizerAgent, SummaryRepository, 3 prompts, 3 specs, 1 fixture |
| Nouveaux tests | +20 (9 review-revise + 6 summarizer + 2 wiring + 3 intégration) |

### Décisions notables
- **Pipeline 12 passes** : `split → pre_translate → translate → consistency → lexicon → grammar → style → polish → review → revise → qa → export`. Compatible modèle immuable par snapshots (choix SDD).
- **Summarizer transverse** (hors STAGES) : évite l'explosion du coût (pas de résumé par retry). Injecté en contexte uniquement pour les chapitres suivants.
- **`reviewLoopEnabled` / `summarizerEnabled`** : toggles utilisateur (défaut `true`). Mode rapide = pipeline 10 passes original.
- **Câblage plugin** : opt-in (fallback built-in intact). Un plugin peut désormais `registerAgent("translate", factory)` et le workflow l'utilisera.

### Open Questions
- **Validation runtime complète** : les tests mockent le LLM. La validation sur vraie Ollama (importer un vrai roman en→fr, observer le score QA réel, itérer les prompts) reste à faire par l'utilisateur.
- **Qualité des prompts review/revise** : facteur de succès clé — itération empirique nécessaire selon les défauts observés sur de vrais textes.
...
## Request — Consolidation du SDD maître (2026-08-05, branche `main`)

### Périmètre
- Fusion exclusive de `NovelTrad_SDD_01-05.md`, `NovelTrad_SDD_06-10.md`,
  `NovelTrad_SDD_11-15.md`, `NovelTrad_SDD_16-20.md` et
  `NovelTrad_SDD_enrichissements.md`.
- `NovelTrad_SDD.md` devient l'unique SDD du dépôt.

### Résultat
- 20 chapitres ordonnés et 208 sections à numérotation continue.
- Enrichissements intégrés dans leur chapitre ; doublons 16.9 à 16.12 éliminés.
- États des jobs, politique de reprise et identifiants d'exigences harmonisés.
- Cinq fichiers SDD intermédiaires supprimés.

### Vérification
- Contrôle structurel dédié : chapitres, sections, doublons, terminologie et invariants.
- `git diff --check` : aucune erreur.

### Handoff
→ Utiliser uniquement `NovelTrad_SDD.md` pour toute évolution future du SDD.
