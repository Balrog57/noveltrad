...
## 2024-05-24 - CSV Parsing Optimization
**Learning:** Loading unbounded or large CSV glossaries entirely into memory at once with list comprehensions creates severe CPU/memory bottlenecks during parsing, and degrades performance when rendering in large applications.
**Action:** When parsing unbounded or large CSV files in Python (e.g., glossaries), avoid list comprehensions for loading all rows (`[row for row in reader]`). Use a single-pass iterator (`iter(csv.reader(...))`) to prevent memory spikes and maintain O(1) complexity.
