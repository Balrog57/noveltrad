...
## 2024-08-01 - CSV Parsing Memory Bottleneck Optimization
**Learning:** Loading all rows from unbounded or large CSV files into memory simultaneously using a list comprehension (`[row for row in reader]`) creates a severe CPU/memory bottleneck, increasing peak intermediate memory usage, which can cause out-of-memory errors on large glossaries.
**Action:** Use a single-pass iterator (`iter(csv.reader(...))`) and process rows generator-style to reduce peak intermediate memory usage and prevent excessive memory usage.
