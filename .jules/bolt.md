...
## 2024-10-24 - CSV Parsing Memory Allocation
**Learning:** Loading all rows into an intermediate list during CSV parsing creates redundant O(N) memory allocations, especially inefficient for unbounded or large files.
**Action:** Avoid list comprehensions for CSV readers and use direct iterator iteration (`for row in reader:`) instead.
