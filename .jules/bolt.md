...
## 2026-07-14 - Lexicon Import Bulk Transaction
**Learning:** SQLite `BEGIN TRANSACTION` does not support nested transactions. Trying to wrap `db.exec('BEGIN TRANSACTION')` at the IPC layer while internal Repository methods (`syncAliases`) also execute transactions leads to crashes.
**Action:** When implementing bulk batch transactions, encapsulate the logic completely within a repository method (e.g. `importMany`) and use flags (`useTransaction = false`) or direct SQL to prevent nested transaction errors while still avoiding the N+1 overhead.
