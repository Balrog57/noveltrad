...
def test_translated_epub_has_no_install_identifier(tmp_path, echo_llm):
...
def test_plain_extractor_docx_stamp_has_no_hex_tail(tmp_path):
    """`build_minimal_docx` never writes an identifying stamp."""
...
def test_html_converter_docx_stamp_has_no_hex_tail(tmp_path):
    """`DocxHtmlConverter.from_html` never writes an identifying stamp."""
    source = fixtures.build_docx(tmp_path)
