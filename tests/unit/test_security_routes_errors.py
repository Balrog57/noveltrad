"""Regression tests for security route error responses.

Handlers previously echoed str(exception) in a ``details`` field, which can
...
def test_upload_hides_security_error_details(security_client):
    ...
    with patch(
        ...
        resp = security_client.post(
            "/api/upload",
            data={"file": (io.BytesIO(b"content"), "book.txt")},
...
def test_verify_upload_hides_exception_details(security_client):
    secret = "/secret/internal/path/leaked"

    with patch(
        "src.api.blueprints.security_routes.Path",
