...
class StageWidget(QFrame):
    ...
    def __init__(self, stage_id: str, label: str) -> None:
        ...
        configure(
            self.expand_btn,
            accessible_name=f"Réduire {self._stage_label}",
            tooltip=f"Réduire {self._stage_label}",
        )
    ...
    def _on_toggle(self, checked: bool) -> None:
        ...
        # Dynamic accessible name update (A11y/Palette requirement)
        if checked:
            configure(
                self.expand_btn,
                accessible_name=f"Réduire {self._stage_label}",
            ...
            configure(
                self.expand_btn,
                accessible_name=f"Développer {self._stage_label}",
                tooltip=f"Développer {self._stage_label}",
            )
