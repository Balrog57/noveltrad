...
class StageWidget(QFrame):
    ...
    def __init__(self, stage_id: str, label: str) -> None:
        ...
        # Initial a11y config based on the default checked state (expanded)
        configure(
            self.expand_btn,
            accessible_name=f"Réduire {self.stage_label}",
            tooltip=f"Réduire {self.stage_label}"
        )
    ...
    def _on_toggle(self, checked: bool) -> None:
        ...
        # Dynamically update a11y based on toggle state
        action_verb = "Réduire" if checked else "Développer"
        configure(
            self.expand_btn,
            accessible_name=f"{action_verb} {self.stage_label}",
            tooltip=f"{action_verb} {self.stage_label}"
        )
