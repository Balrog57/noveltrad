...
class NLLBEngine:
    ...
    def _translate_multiblock(
        ...
        # Batch encode paragraphs natively
        batched_tokens = self._sp.encode(para_texts, out_type=str)  # type: ignore[union-attr]
        source_batch = [[src_code, *tokens, "</s>"] for tokens in batched_tokens]
        ...
        # Gather output tokens and batch decode natively
        out_tokens_batch = []
        for res in results:
        ...
        decoded_batch = self._sp_target.decode(out_tokens_batch)  # type: ignore[union-attr]

        # Fill in translated paragraphs
        for j, idx in enumerate(para_indices):
            output_parts[idx] = decoded_batch[j]
    ...
    def translate_batch(
        ...
        with self._lock:
            try:
                ...
                source_tokens_batch = [
                    [src_code, *tokens, "</s>"] for tokens in batched_tokens
                ]
