...
class NLLBEngine:
    ...
    def _translate_multiblock(
        ...
        # ⚡ Bolt Optimization: Use native batch encode
        ...
        # ⚡ Bolt Optimization: Use native batch decode
        all_out_tokens = []
        for res in results:
        ...
        decoded_texts = self._sp_target.decode(all_out_tokens)  # type: ignore[union-attr]

        # Fill in translated paragraphs
        for j, idx in enumerate(para_indices):
            output_parts[idx] = decoded_texts[j]
    ...
    def translate_batch(
        ...
        with self._lock:
            try:
                ...
                # Tokenize all non-empty texts natively
                source_tokens_batch = []
