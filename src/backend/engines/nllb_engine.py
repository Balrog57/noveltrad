...
class NLLBEngine:
    ...
    def _translate_multiblock(
        ...
        # Gather tokens for batched decoding
        out_tokens_batch = []
    ...
    def translate_batch(
        ...
        with self._lock:
            try:
                ...
                # Native batched encode
                texts_to_encode = [t.strip() for t in non_empty_texts]
                tokens_batch = self._sp.encode(texts_to_encode, out_type=str)  # type: ignore[union-attr]
                for tokens in tokens_batch:
