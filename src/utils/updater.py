...
def perform_replace_and_relaunch(new_dir: Path, install_dir: Path | None = None) -> Path:
    ...
    # 🛡️ Sentinel: Create temp batch file securely using mkstemp to avoid TOCTOU
    fd, temp_path = tempfile.mkstemp(prefix="noveltrad_updater_", suffix=".bat", text=True)
    bat_path = Path(temp_path)
