...
Tokens are stored next to the durable ``.env`` (``data/chatgpt_oauth.json``)
and never logged. The OAuth client id is the public Codex CLI id — OpenAI's
...
from src.config import primary_env_file_path
...
# Codex registers this redirect; NovelTrad's Flask port cannot be substituted.
...
def oauth_store_path() -> Path:
    """Durable token file, same directory rule as ``primary_env_file_path``."""
    env_path = primary_env_file_path()
    return env_path.parent / "chatgpt_oauth.json"
...
def import_codex_auth(codex_path: Optional[Path] = None, dest: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """Reuse a local Codex CLI login when NovelTrad has no tokens yet."""
