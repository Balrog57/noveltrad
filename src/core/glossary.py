...
def _parse_json(text: str) -> dict[str, str]:
    ...
    if isinstance(data, list):
        ...
        for i, item in enumerate(data):
            ...
            translation = item.get("translation") or item.get("target") or item.get("value")
            if term is None or translation is None:
                raise GlossaryError(f"Glossary entry #{i} missing term/translation: {item!r}")
...
def _parse_delimited(text: str, delimiter: str) -> dict[str, str]:
    ...
    for row in reader:
        ...
        if not row or len(row) < 2 or not any(cell.strip() for cell in row):
            continue
        ...
        if is_first:
            ...
            header_like = (
                row[0].strip().lower() in term_headers and row[1].strip().lower() in trans_headers
            )
            if header_like:
                continue
